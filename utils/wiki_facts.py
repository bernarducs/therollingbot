...
def download_wiki_albums_summary():
    ...
    albums_list.pop()
    first_albums = [
        i[0] for i in re.findall(r'([\w\s\']+) (\w*)', f'{albums_list[0]}')
    ]
    ...
    for album in albums_list:
        ...
        doc = nlp(summary)
        with open(
            f'data/wiki/summary/albums/{album_name_file}.txt',
            'w',
            encoding='utf-8',
        ) as f:
...
def download_wiki_lyrics_summary():
    ...
    links_and_songlyrics = [
        (x, z)
        for x in links
        for y in songs
        for z in lyrics_songs
